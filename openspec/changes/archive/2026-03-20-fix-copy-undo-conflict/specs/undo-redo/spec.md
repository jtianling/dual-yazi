...
- **THEN** an undo entry of type Copy { pairs: [], overwritten: [] } SHALL be pushed immediately
- **AND** the actual destination paths SHALL be populated via scheduler hook callbacks as each file copy completes

#### Scenario: Recording a cut-paste operation
- **WHEN** user pastes yanked files (cut mode) into a destination directory
- **THEN** an undo entry of type Move { pairs: [], overwritten: [] } SHALL be pushed immediately
- **AND** the actual destination paths SHALL be populated via scheduler hook callbacks as each file cut completes

#### Scenario: Recording a copy_to cross-pane operation
- **WHEN** user executes copy_to (copying selected files to the other pane)
- **THEN** an undo entry of type Copy { pairs: [], overwritten: [] } SHALL be pushed immediately
- **AND** the actual destination paths SHALL be populated via scheduler hook callbacks
...
- **WHEN** user presses `u` and the top undo entry is Copy { pairs, overwritten }
- **THEN** the system SHALL delete all destination files from the `pairs` list
- **AND** for each entry in `overwritten`, the system SHALL restore the original file from its trash path
- **AND** the entry SHALL be moved to the redo stack

#### Scenario: Undo a move
- **WHEN** user presses `u` and the top undo entry is Move { pairs, overwritten }
...
- **WHEN** user presses `<C-r>` and the top redo entry is Copy { pairs, overwritten }
- **THEN** the system SHALL re-copy each source to its destination via the Scheduler (with force=true)
- **AND** the entry SHALL be moved back to the undo stack

#### Scenario: Redo a move
- **WHEN** user presses `<C-r>` and the top redo entry is Move { pairs, overwritten }
