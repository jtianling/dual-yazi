...
#[derive(Debug)]
pub struct UndoPushMoveOverwrittenOpt {
	pub original:   Option<UrlBuf>,
	pub trash_path: Option<UrlBuf>,
}

impl From<ActionCow> for UndoPushMoveOverwrittenOpt {
	fn from(mut a: ActionCow) -> Self {
		Self { original: a.take_any("original"), trash_path: a.take_any("trash_path") }
	}
}

impl FromLua for UndoPushMoveOverwrittenOpt {
	fn from_lua(_: Value, _: &Lua) -> mlua::Result<Self> { Err("unsupported".into_lua_err()) }
}

impl IntoLua for UndoPushMoveOverwrittenOpt {
